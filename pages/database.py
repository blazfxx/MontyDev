...
def save_summary(username, original, summary):
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('INSERT INTO summaries (username, original_text, summary_text) VALUES (?, ?, ?)', (username, original, summary))
        conn.commit()
        conn.close()
        return True
    except:
...
def load_summaries(username):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('SELECT original_text, summary_text, created_at FROM summaries WHERE username = ? ORDER BY created_at DESC', (username,))
    rows = cursor.fetchall()
    conn.close()
    return rows